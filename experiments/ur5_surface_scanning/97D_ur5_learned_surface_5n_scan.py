#!/usr/bin/env python3
"""97D - UR5 learned-surface full-path scan with 5 N normal-force control.
...
6. Execute the complete 200 x 120 mm, 1.539 m raster path while maintaining
   5 N normal force.
7. Use FT300 tangent-plane moments to compliantly correct tool roll/pitch while
   keeping the learned normal and path orientation as the nominal reference.

Desired positions and surface normals come ONLY from the learned reconstruction
produced by step 97B. The analytic surface is not used for any desired pose.
Step 97C supplies the verified IK seed; force/contact control reuses the validated
step-95/96 controller.
"""
...
OUTPUT_PATH = HERE / "97D_learned_surface_force_scan_result.npz"
...
# ---------------------------------------------------------------------------
# Moment-based orientation admittance
# ---------------------------------------------------------------------------

# The learned 97B orientation remains the nominal reference.  Only local tool
# X/Y tilt is compliant; local Z/yaw stays on the learned raster orientation.
TILT_INERTIA = 0.08
TILT_DAMPING = 1.20
TILT_STIFFNESS = 8.0
...
def validate_learned_path(
    ...
    for key in path_keys:
        if len(geometry[key]) != count:
            raise RuntimeError(
                f"97B字段长度不一致: {key}"
            )

        if len(ik_data[key]) != count:
            raise RuntimeError(
                "97C没有覆盖97B完整路径: "
                f"{key}={len(ik_data[key])}, "
                f"expected={count}"
            )

    if len(ik_data["q"]) != count:
        raise RuntimeError(
            "97C IK解没有覆盖97B完整路径: "
            f"q={len(ik_data['q'])}, expected={count}"
        )
    ...
    for key in [
        ...
        if not np.all(
            np.isfinite(geometry[key])
        ):
            raise RuntimeError(
                f"97B字段包含NaN/Inf: {key}"
            )

    # 97C会原样保存它实际验证的97B轨迹。严格比较可防止误用旧IK文件。
    for key in path_keys:
    ...
    if positions.shape != (count, 3):
        raise RuntimeError(
            f"97B position形状错误: {positions.shape}"
        )

    if normals.shape != (count, 3):
        raise RuntimeError(
            f"97B surface_normal形状错误: {normals.shape}"
        )

    if rotations.shape != (count, 3, 3):
        raise RuntimeError(
            f"97B rotation形状错误: {rotations.shape}"
        )

    if arc_length.shape != (count,):
        raise RuntimeError(
            f"97B arc_length形状错误: {arc_length.shape}"
        )

    if np.any(
        np.diff(arc_length) <= 0.0
    ):
        raise RuntimeError(
            "97B arc_length必须严格递增"
        )
    ...
    if np.max(
        np.abs(normal_norms - 1.0)
    ) > 1e-6:
        raise RuntimeError(
            "97B曲面法向不是单位向量"
        )

    tool_normal_alignment = np.einsum(
        "ij,ij->i",
        rotations[:, :, 2],
        -normals,
    )

    if np.min(tool_normal_alignment) < 1.0 - 1e-8:
        raise RuntimeError(
            "97B姿态的工具+Z轴没有与负曲面法向对齐"
        )
...
def main():
    ...
    # Local learned-tool-frame roll/pitch correction [rx, ry].
    ...
    tcp_errors_scan = []
    normal_errors_scan = []
    learned_normal_deviations_scan = []
    tangential_forces_scan = []
    tilt_moment_norms_scan = []
    tilt_angles_scan = []
    ...
    desired_force_scan = []
    measured_force_scan = []
    desired_position_scan = []
    actual_position_scan = []
    learned_normal_scan = []
    actual_tool_z_scan = []
    actuator_torque_scan = []
    qpos_scan = []
    measured_moment_world_scan = []
    sensor_moment_world_scan = []
    measured_tilt_moment_scan = []
    tilt_correction_scan = []
    tilt_velocity_scan = []
    ...
    print(
        "97C role 97C用途: full-path verified IK seed "
        "全路径验证后的初始IK种子"
    )
    ...
    with mujoco.viewer.launch_passive(
        ...
        while viewer.is_running():
            ...
            # ------------------------------------------------------------
            # FT300 moment -> roll/pitch orientation admittance
            # ------------------------------------------------------------
            nominal_rotation = (
            ...
            # Shift the FT300 wrench from the sensor origin to the probe TCP:
            # M_tcp = M_sensor - r_sensor->tcp x F.  Without this shift,
            # tangential friction is mistaken for an attitude error.
            moment_world = (
                sensor_moment_world
                - np.cross(
                    sensor_to_tip_world,
            ...
            # Only moments about learned tool X/Y adjust attitude.  Moment
            # about tool Z is intentionally rejected so friction cannot spin
            # the tool away from the learned raster direction.
            tilt_moment = (
                moment_nominal[:2]
                .copy()
            )
            ...
            correction_rotation_vector = np.array(
                [
                    tilt_correction[0],
                    tilt_correction[1],
                    0.0,
            ...
            correction_velocity_local = np.array(
                [
                    tilt_velocity[0],
                    tilt_velocity[1],
                    0.0,
            ...
            correction_acceleration_local = np.array(
                [
                    tilt_acceleration[0],
                    tilt_acceleration[1],
                    0.0,
            ...
            correction_velocity_world = (
                nominal_rotation
                @ correction_velocity_local
            )

            correction_acceleration_world = (
                nominal_rotation
                @ correction_acceleration_local
                + np.cross(
                    desired_omega,
                    correction_velocity_world,
                )
            ...
            learned_normal_alignment = float(
                np.clip(
                    np.dot(
                        tool_z_actual,
                        -current_normal,
            ...
            learned_normal_deviation_deg = float(
                np.degrees(
                    np.arccos(
                        learned_normal_alignment
                    )
                )
            )

            tilt_moment_norm = float(
                np.linalg.norm(
                    tilt_moment
                )
            )
            ...
            if (
                ...
                learned_normal_deviations_scan.append(
                ...
                tilt_moment_norms_scan.append(
                    tilt_moment_norm
                )

                tilt_angles_scan.append(
                    tilt_angle_deg
                )
                ...
                progress_scan.append(
                    progress_percent
                )

                desired_force_scan.append(
                    desired_force
                )

                measured_force_scan.append(
                    measured_normal_force
                )

                desired_position_scan.append(
                    p_des.copy()
                )

                actual_position_scan.append(
                    p_actual.copy()
                )

                learned_normal_scan.append(
                    current_normal.copy()
                )

                actual_tool_z_scan.append(
                    tool_z_actual.copy()
                )

                actuator_torque_scan.append(
                    tau_act.copy()
                )
                ...
                measured_moment_world_scan.append(
                    moment_world.copy()
                )

                sensor_moment_world_scan.append(
                    sensor_moment_world.copy()
                )

                measured_tilt_moment_scan.append(
                    tilt_moment.copy()
                )

                tilt_correction_scan.append(
                    tilt_correction.copy()
                )

                tilt_velocity_scan.append(
                    tilt_velocity.copy()
                )
            ...
            if (
                ...
                print(
                    ...
                    f"Mxy={tilt_moment_norm:.4f} Nm | "
                    f"tilt={tilt_angle_deg:.3f} deg | "
            ...
            if (
                ...
                if force_errors_scan:
                    ...
                    print(
                        f"Corrected orientation mean/max tracking error "
                        f"修正姿态平均/最大跟踪误差: "
                    ...
                    print(
                        f"Learned-normal mean/max deviation "
                        f"相对学习法向平均/最大修正角: "
                        f"{np.mean(learned_normal_deviations_scan):.3f} / "
                        f"{np.max(learned_normal_deviations_scan):.3f} deg"
                    )

                    print(
                        f"FT300 tilt moment mean/max "
                        f"FT300倾斜力矩平均/最大值: "
                        f"{np.mean(tilt_moment_norms_scan):.4f} / "
                        f"{np.max(tilt_moment_norms_scan):.4f} Nm"
                    )

                    print(
                        f"Orientation admittance mean/max correction "
                        f"姿态导纳平均/最大修正角: "
                        f"{np.mean(tilt_angles_scan):.3f} / "
                        f"{np.max(tilt_angles_scan):.3f} deg"
                    )
                ...
                result_pass = bool(
                    force_errors_scan
                    and np.mean(force_errors_scan)
                    <= PASS_FORCE_MEAN_ERROR
                    and np.max(force_errors_scan)
                    <= PASS_FORCE_MAX_ERROR
                    and np.max(tcp_errors_scan)
                    <= PASS_TCP_MAX_ERROR
                    and np.max(normal_errors_scan)
                    <= PASS_NORMAL_MAX_ERROR_DEG
                    and saturation_ratio
                    <= PASS_SATURATION_RATIO
                )

                np.savez(
                    OUTPUT_PATH,
                    source_97b=np.asarray(
                        GEOMETRY_PATH.name
                    ),
                    source_97c=np.asarray(
                        IK_PATH.name
                    ),
                    analytic_surface_used=np.asarray(
                        False
                    ),
                    result_pass=np.asarray(
                        result_pass
                    ),
                    max_actuator_torque=np.asarray(
                        max_torque
                    ),
                    saturation_ratio_percent=np.asarray(
                        saturation_ratio
                    ),
                    ...
                    progress_percent=np.asarray(
                        progress_scan
                    ),
                    desired_force=np.asarray(
                        desired_force_scan
                    ),
                    measured_normal_force=np.asarray(
                        measured_force_scan
                    ),
                    ...
                    learned_normal=np.asarray(
                        learned_normal_scan
                    ),
                    actual_tool_z=np.asarray(
                        actual_tool_z_scan
                    ),
                    ...
                    normal_error_deg=np.asarray(
                        normal_errors_scan
                    ),
                    learned_normal_deviation_deg=np.asarray(
                        learned_normal_deviations_scan
                    ),
                    tangential_force=np.asarray(
                        tangential_forces_scan
                    ),
                    tilt_moment_norm=np.asarray(
                        tilt_moment_norms_scan
                    ),
                    tilt_angle_deg=np.asarray(
                        tilt_angles_scan
                    ),
                    measured_moment_world=np.asarray(
                        measured_moment_world_scan
                    ),
                    sensor_moment_world=np.asarray(
                        sensor_moment_world_scan
                    ),
                    measured_tilt_moment=np.asarray(
                        measured_tilt_moment_scan
                    ),
                    tilt_correction=np.asarray(
                        tilt_correction_scan
                    ),
                    tilt_velocity=np.asarray(
                        tilt_velocity_scan
                    ),
                    normal_correction=np.asarray(
                        normal_corrections_scan
                    ),
