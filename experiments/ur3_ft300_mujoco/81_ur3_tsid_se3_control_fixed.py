...
3. 使用 TrajectorySample(12,6) 设置 SE3 reference；
4. 读取 TSID 输出 ddq 与 tau；
...
# ============================================================
# 9.1 SE3 reference sample
#
# Python binding of TSID 1.10.0 exposes:
#
#     TaskSE3Equality.setReference(TrajectorySample)
#
# rather than the C++ convenience overload:
#
#     setReference(pinocchio::SE3)
#
# An SE3 trajectory sample uses:
#
#     value dimension      = 12
#     derivative dimension = 6
#
# The 12D value is TSID's vectorized SE3 representation.
# We use TSID's own SE3ToVector() to avoid guessing the layout.
# ============================================================
...
if not hasattr(
    ...
    raise RuntimeError(
        "当前 TSID Python 模块没有 SE3ToVector；"
        "本实验需要 TSID 1.7+ / 1.10.0"
    )
...
if se3_reference_value.shape != (12,):
    raise RuntimeError(
        "SE3ToVector 返回维度异常: "
        f"{se3_reference_value.shape}"
    )


# 为了兼容 TSID 1.10.0 的 Python binding，
# 这里使用仍然可用的 pos/vel/acc setter。
# 它们虽然会被新版标记为 deprecated，
# 但行为明确且与 80A 已验证一致。
