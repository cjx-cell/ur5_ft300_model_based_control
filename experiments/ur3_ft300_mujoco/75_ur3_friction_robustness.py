...
- 保持学习轨迹、工件位置、控制器参数不变
- 只把第二遍回放的滑动摩擦系数从 0.05 提高到 0.30
- FT300 继续负责 5 N 法向力修正
- 额外统计切向力，确认摩擦扰动确实进入系统
- 控制器不知道摩擦系数变化
"""
...
CONTACT_XML = SCRIPT_DIR / "ur3_ft300_friction_robustness.xml"
...
# ============================================================
# 75B single-variable perturbation
#
# 74A 扫描时滑动摩擦系数为 0.05。
# 75B 第二遍回放时，把真实接触滑动摩擦系数提高到 0.30。
#
# 其他条件保持不变：
# - 工件位置不变
# - learned path 不变
...
baseline_sliding_friction = 0.05
replay_sliding_friction = 0.30
...
print(
    f"Sliding friction 扫描/回放滑动摩擦系数: "
    f"{baseline_sliding_friction:.2f} / "
    f"{replay_sliding_friction:.2f}"
)
...
with mujoco.viewer.launch_passive(
    ...
    while viewer.is_running():
        ...
        current_learned_tangent = np.array([
            -current_learned_normal[1],
            current_learned_normal[0],
            0.0,
        ])

        current_learned_tangent = normalize(
            current_learned_tangent
        )

        measured_tangential_force = abs(
            float(
                np.dot(
                    external_world_wrench[:3],
                    current_learned_tangent,
        ...
        if replay_start <= current_time <= replay_end:
            ...
            replay_tangential_forces.append(
                measured_tangential_force
            )

            if measured_normal_force > 0.5:
                replay_force_ratios.append(
                    measured_tangential_force
                    /
                    measured_normal_force
                )
        ...
        if (
            ...
            print(
                ...
                f"切向力平均/最大: "
                f"{np.mean(replay_tangential_forces):.3f} / "
                f"{np.max(replay_tangential_forces):.3f} N"
            )

            if len(replay_force_ratios) > 0:
                print(
                    f"|Ft|/Fn mean/max "
                    f"切法向力比平均/最大: "
                    f"{np.mean(replay_force_ratios):.3f} / "
                    f"{np.max(replay_force_ratios):.3f}"
                )

            print(
                f"Sliding friction change "
                f"滑动摩擦系数变化: "
                f"{baseline_sliding_friction:.2f} -> "
                f"{replay_sliding_friction:.2f}"
            )
        ...
        if step >= 1000 and step % 1000 == 0:
            ...
            print(
                f"Tangential force 切向力: "
                f"{measured_tangential_force:.3f} N"
            )
