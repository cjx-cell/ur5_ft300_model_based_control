#!/usr/bin/env python3
"""
75_ur3_scan_noise_robustness.py
...
CONTACT_XML = SCRIPT_DIR / "ur3_ft300_scan_noise_robustness.xml"
...
# ============================================================
# 75D single-variable perturbation
#
# 模拟扫描/几何重建阶段额外出现位置噪声。
#
# 唯一扰动：
...
scan_noise_rng = np.random.default_rng(scan_noise_seed)
...
# 1) 在74A学习位置上注入确定性噪声。
noisy_positions = clean_learned_positions.copy()

noisy_positions[:, 0] += scan_noise_rng.normal(
    0.0,
    scan_position_noise_std,
    len(noisy_positions),
)

noisy_positions[:, 1] += scan_noise_rng.normal(
    0.0,
    scan_position_noise_std,
    len(noisy_positions),
)

# 当前实验曲面位于XY平面，因此Z不额外加噪，
# 避免把二维轮廓扫描误差和离面误差混在一起。

# 2) 重新平滑轨迹。
learned_positions = local_polynomial_smooth(
    learned_s,
    noisy_positions,
    half_window=10,
    polynomial_degree=3,
...
# 3) 从带噪重建轨迹重新计算切向和法向。
noisy_derivative = np.zeros_like(
    learned_positions,
    dtype=float,
)

for axis in range(3):
    noisy_derivative[:, axis] = np.gradient(
        learned_positions[:, axis],
        learned_s,
        edge_order=2,
    )
...
previous_tangent = initial_tangent_world.copy()
previous_normal = initial_normal_world.copy()

for i in range(len(learned_s)):

    tangent = noisy_derivative[i].copy()
    tangent[2] = 0.0
    ...
    reconstructed_tangents[i] = tangent
    reconstructed_normals[i] = normal

    previous_tangent = tangent
    previous_normal = normal

learned_tangents = reconstructed_tangents
learned_normals = reconstructed_normals

# 4) 评价注入噪声后几何本身发生了多少变化。
...
for noisy_normal, clean_normal in zip(
    learned_normals,
