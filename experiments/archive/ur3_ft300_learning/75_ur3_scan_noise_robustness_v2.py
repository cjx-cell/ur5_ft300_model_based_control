...
- 向学习位置加入 0.15 mm 标准差的 XY 高斯噪声
- 使用 ±3 mm 局部二次回归直接估计位置与切向
- 由稳健切向重建法向，不使用相邻点差分求导
- FT300 继续负责 5 N 法向力修正
- 真实工件、摩擦和工具尺寸保持74B基准
- 控制器不使用隐藏曲面真值
"""
...
CONTACT_XML = SCRIPT_DIR / "ur3_ft300_scan_noise_robustness_v2.xml"
...
# 74A 原始学习几何，仅用于评价噪声扰动后的重建误差。
clean_learned_positions = learned_positions.copy()
clean_learned_normals = learned_normals.copy()
...
# ============================================================
# 75D-v2 single-variable perturbation
#
# 模拟扫描阶段额外出现位置测量噪声：
#
#     XY 高斯噪声标准差 = 0.15 mm
...
# 与上一版75D不同：
#
# 上一版先平滑位置，再直接用相邻点 np.gradient 求导。
# 当点间距约0.10 mm，而噪声标准差达到0.15 mm时，
# “差分求导”会强烈放大噪声，尤其在路径首尾。
#
# 本版采用：
#
#     局部二次多项式回归
#
# 并且直接从局部拟合多项式的一阶导数得到切向，
# 不再对带噪位置做相邻点差分。
#
# 其他条件保持74B基准不变：
#
...
scan_position_noise_std = 0.00015

scan_noise_seed = 20260917

scan_noise_rng = np.random.default_rng(
    scan_noise_seed
)


# 74A 学习路径约30.66 mm / 300段，
# 单点间距约0.10 mm。
#
# 这里用 ±3 mm 空间窗口进行局部拟合，
# 即一个内部点通常使用约60个邻域点。
geometry_fit_half_window = 0.0030

geometry_fit_degree = 2


def robust_local_geometry_fit(
    s_values,
    measured_positions,
):
    ...
    fitted_positions = np.zeros_like(
        measured_positions,
        dtype=float,
    )

    fitted_derivatives = np.zeros_like(
        measured_positions,
        dtype=float,
    )
...
# ============================================================
# 1. Add deterministic XY position noise
# ============================================================

noisy_positions = (
    clean_learned_positions.copy()
)
...
noisy_positions[
    :,
    0
] += scan_noise_rng.normal(
    0.0,
    scan_position_noise_std,
    len(
        noisy_positions
    ),
)
...
# ============================================================
# 2. Robust local geometry reconstruction
# ============================================================

(
    learned_positions,
    fitted_path_derivatives,
) = robust_local_geometry_fit(
    learned_s,
    noisy_positions,
)
...
# ============================================================
# 3. Reconstruct tangent + normal
# ============================================================

reconstructed_tangents = np.zeros_like(
    learned_positions,
    dtype=float,
)

reconstructed_normals = np.zeros_like(
    learned_positions,
    dtype=float,
)
...
for i in range(
    ...
    # 如果某一点局部拟合退化，不允许产生随机方向，
    # 直接沿用上一点切向。
    if tangent_norm < 1e-6:

        tangent = (
            previous_tangent.copy()
        )
...
# ============================================================
# 4. Evaluate geometry perturbation
#
# 仅用于实验评价，不参与控制。
# ============================================================

scan_geometry_position_error = np.linalg.norm(
    learned_positions
    -
    clean_learned_positions,
    axis=1,
)


scan_geometry_normal_error = []
...
for (
    ...
    scan_geometry_normal_error.append(
        np.rad2deg(
            np.arccos(
                alignment
            )
...
scan_geometry_normal_error = np.asarray(
    scan_geometry_normal_error,
    dtype=float,
)


# 如果重建出现几十度甚至接近180度的法向错误，
# 说明测试管线本身失效，而不是在测试控制器鲁棒性。
#
# 这里提前阻止这种无效实验继续运行。
if (
    np.max(
    ...
    raise RuntimeError(
        "扫描几何重建出现超过20 deg的法向误差；"
        "这属于几何估计失败，不应继续作为控制鲁棒性实验。"
    )
...
print(
    f"Scan position noise std "
    f"扫描位置噪声标准差: "
    f"{scan_position_noise_std * 1000.0:.3f} mm"
)

print(
    f"Nominal sample spacing "
    f"名义采样点间距: "
    f"{np.mean(np.diff(learned_s)) * 1000.0:.3f} mm"
)

print(
    f"Geometry fit half-window "
    f"几何拟合半窗口: "
    f"{geometry_fit_half_window * 1000.0:.1f} mm"
)

print(
    f"Geometry position mean/max error "
    f"重建轨迹平均/最大偏差: "
    f"{np.mean(scan_geometry_position_error) * 1000.0:.3f} / "
    f"{np.max(scan_geometry_position_error) * 1000.0:.3f} mm"
)

print(
    f"Geometry normal mean/max error "
    f"重建法向平均/最大偏差: "
    f"{np.mean(scan_geometry_normal_error):.3f} / "
    f"{np.max(scan_geometry_normal_error):.3f} deg"
)

print(
    "Perturbation type 扰动类型: "
    "single variable 单变量"
)
