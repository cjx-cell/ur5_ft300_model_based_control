...
- 保持工件位置、学习轨迹、学习法向和控制参数不变
- 只把第二遍加工的实际球头半径减少 0.5 mm
...
CONTACT_XML = SCRIPT_DIR / "ur3_ft300_tool_radius_robustness.xml"
...
# ============================================================
# 75C single-variable perturbation
#
# 74A 扫描时球形探针半径由 learned file 给出，通常是 5.0 mm。
# 第二遍加工时，假设工具发生磨损，实际半径减少 0.5 mm。
#
# 注意：
# - learned path 不变；
# - learned normal 不变；
...
# - 5 N 力控参数不变；
# - 控制器不知道工具半径已经变化；
# - 只有 MuJoCo 中真实接触球半径改变。
#
# 由于 learned path 记录的是旧工具的球心轨迹，
# 更小的实际球头必须沿局部法向再向工件靠近，
# 才能恢复相同的接触。
# ============================================================

scan_probe_radius = probe_radius
tool_radius_change = -0.0005
actual_probe_radius = scan_probe_radius + tool_radius_change

if actual_probe_radius <= 0.0:
    raise RuntimeError("实际探针半径必须大于0")
...
print(
    f"Probe radius 扫描/实际工具半径: "
    f"{scan_probe_radius * 1000.0:.2f} / "
    f"{actual_probe_radius * 1000.0:.2f} mm"
)

print(
    f"Tool radius change 工具半径变化: "
    f"{tool_radius_change * 1000.0:+.2f} mm"
)
...
with mujoco.viewer.launch_passive(
    ...
    while viewer.is_running():
        ...
        if replay_start <= current_time <= replay_end:
            ...
            replay_force_corrections.append(
                normal_correction
            )
        ...
        if (
            ...
            print(
                f"Force correction mean/min/max "
                f"法向力控修正平均/最小/最大: "
                f"{np.mean(replay_force_corrections) * 1000.0:+.3f} / "
                f"{np.min(replay_force_corrections) * 1000.0:+.3f} / "
                f"{np.max(replay_force_corrections) * 1000.0:+.3f} mm"
            )

            print(
                f"Probe radius scan/actual "
                f"扫描/实际工具半径: "
                f"{scan_probe_radius * 1000.0:.3f} / "
                f"{actual_probe_radius * 1000.0:.3f} mm"
            )
