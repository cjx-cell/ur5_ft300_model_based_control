...
CONTACT_XML = SCRIPT_DIR / "ur3_ft300_workpiece_offset_robustness.xml"
...
# ============================================================
# 75A single-variable perturbation
#
# 第一遍74A扫描完成后，假设实际工件在第二遍加工前
# 沿初始表面法向整体向前移动 1.5 mm。
#
# 注意：
# - learned path 保持不变；
# - controller 不知道这个偏差；
# - 只有 MuJoCo 真实工件被移动；
# - FT300 必须通过 normal_correction 自己补偿。
# ============================================================

workpiece_normal_offset = 0.0015
...
print(
    "\n"
    "========== 75A Workpiece Offset Robustness "
    "75A工件偏差鲁棒性 =========="
)
...
print(
    f"Workpiece normal offset "
    f"工件法向位置偏差: "
    f"{workpiece_normal_offset * 1000.0:+.2f} mm"
)
...
with mujoco.viewer.launch_passive(
    ...
    while viewer.is_running():
        ...
        if (
            ...
            print(
                f"Applied workpiece offset "
                f"施加工件偏差: "
                f"{workpiece_normal_offset * 1000.0:+.3f} mm"
            )
